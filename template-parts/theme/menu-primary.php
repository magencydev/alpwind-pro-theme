...
<nav class="sticky transition-transform transform z-40 top-0 left-0 w-full xl:text-lg"
	...
	<div class="container mx-auto px-4 flex flex-wrap justify-between items-center py-4 xl:py-6"
		...
		<div :class="drawer_container_classes" x-cloak
			class="nav-main top-0 left-0 z-10 fixed lg:static h-screen lg:h-auto lg:flex lg:items-center
					...
					<li class="relative px-3 lg:px-4 xl:px-5 py-2 w-full lg:w-auto <?php echo esc_attr( implode( ' ', $item->classes ) ); ?>"
						...
						@mouseleave="active = false">
							...
							<a href="<?php echo esc_url( $item->url ); ?>"
								...
								class="nav-text transition-colors duration-200 flex justify-between items-stretch lg:block
							...
							<?php if ( isset( $item->children ) ) : ?>
								<ul class="nav-dropdown lg:absolute left-0 lg:mt-4 w-56 transition-all duration-200 overflow-hidden"
									...
									:class="{
										'lg:max-h-screen lg:px-4 py-2 lg:py-4': active,
										'lg:max-h-0': !active,
										'max-h-0': !mobile,
										'max-h-full': mobile
									}">
									...
									<?php foreach ( $item->children as $child ) : ?>
										<li class="w-full px-3 py-2">
											<a href="<?php echo esc_url( $child->url ); ?>"
												...
												class="nav-dropdown-text transition-colors duration-200
